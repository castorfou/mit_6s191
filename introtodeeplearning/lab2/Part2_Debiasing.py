...
def vae_loss_function(x, x_recon, mu, logsigma, kl_weight=0.0005):
    ...
    latent_loss = 0.5*(np.sum(np.exp(logsigma)+np.square(mu)-1-logsigma))# TODO
    ...
    reconstruction_loss = tf.reduce_mean(x-x_recon, 0) # TODO
...
def sampling(z_mean, z_logsigma):
    ...
    z = z_mean+np.exp(0.5*z_logsigma)*epsilon# TODO
    return z
...
def debiasing_loss_function(x, x_pred, y, y_logit, mu, logsigma):
    ...
    # samples
    total_loss = tf.reduce_mean(classification_loss + face_indicator*vae_loss, 0) # TODO
...
class DB_VAE(tf.keras.Model):
  ...
  def call(self, x): 
    ...
    z = reparameterize(z_mean, z_logsigma)# TODO
...
def get_training_sample_probabilities(images, dbvae, bins=10, smoothing_fac=0.001): 
    ...
    for i in range(latent_dim):
        ...
        bin_idx = np.digitize(latent_distribution, bins) # TODO
        ...
        training_sample_p = np.max(p, training_sample_p)# TODO
